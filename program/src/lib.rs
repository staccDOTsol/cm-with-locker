...
anchor_lang::declare_id!("Cndy2utXYUAN62pVx82aTNGRSEq2HLfsDxZEKSbLqsHR");
...
pub mod candy_machine {
    ...
    pub fn mint_nft<'info>(
        ...
        if let pass_codes_available = &candy_machine.data.pass_codes_available {
            if let candy_locker = &mut ctx.accounts.candy_locker {
                if let passes_used = &candy_locker.passes_used {
                    if passes_used == pass_codes_available {
                        return Err(ErrorCode::AllPassesConsumed.into());
                    }
                    if let Some(pass_code) = &pass_code {
                        // ADD AES CHECKING FOR THE STRING ABOVE
                        type HmacSha256 = Hmac<Sha256>;
                        // let mut mac = HmacSha256::new_from_slice(PREFIX.as_bytes())
                        // .expect("HMAC can take key of any size");
                        let mut mac = HmacSha256::new_from_slice(PREFIX.as_bytes()).unwrap();
                        mac.update(&pass_code.as_bytes());
                        let result = mac.finalize();
                        let code = result.into_bytes();

                        let candy_locker_account = candy_locker.to_account_info();
                        let data = &candy_locker_account.data.borrow_mut();
                        let length_till_pass_codes = CANDY_LOCKER_CONFIG_START + 4;
                        // let total_pass_code_count = get_pass_code_count(&data).expect("Failed to read line");
                        let total_pass_code_count = get_pass_code_count(&data).unwrap();
                        
                        let mut found = false;
                        let used = false;
                        let position = CANDY_LOCKER_CONFIG_START + 4;

                        for i in 0..total_pass_code_count{
                            let next_position = length_till_pass_codes + (i * PASS_CODE_LINE_SIZE);
                            
                            let pass_code_data = &data[position..next_position-1];
                            if pass_code_data == &*code{
                                found = true;
                            }
                            if used && found{
                                return Err(ErrorCode::PassCodeHasBeenUsed.into());
                            }
                            if found{
                                break;
                            }

                            let position = length_till_pass_codes + ((i-1) * PASS_CODE_LINE_SIZE);
                        }

                        if !found {
                            return Err(ErrorCode::PassCodeNotFound.into());
                        }

                    } else{
                        return Err(ErrorCode::PassCodeNotRecieved.into());
                    }

                } 
            } else{
                return Err(ErrorCode::CandyLockerAccountNotFound.into());
            }
        };
    ...
    pub fn add_pass_code_lines(
        ctx: Context<AddPassCodeLine>,
        index: u32,
        pass_code_lines: Vec<PassCodeLine>,
    ) -> ProgramResult {
        let candy_machine = &mut ctx.accounts.candy_machine;
        let candy_locker = &mut ctx.accounts.candy_locker;
        let account_locker = candy_locker.to_account_info();
        let current_pass_code_count = get_pass_code_count(&account_locker.data.borrow_mut())?;
        let mut data = account_locker.data.borrow_mut();
        let mut fixed_pass_code_lines = vec![];

        for line in &pass_code_lines {
            let pass_code_encrypted = line.pass_code_encrypted.clone();
            let used = line.used.clone();
            fixed_pass_code_lines.push(PassCodeLine { pass_code_encrypted, used })
        }

        let as_vec = fixed_pass_code_lines.try_to_vec()?;

        // remove unneeded u32 because we're just gonna edit the u32 at the front
        // don't know if I should remove this or not. let's assume removed.
        let serialized: &[u8] = &as_vec.as_slice()[4..];

        let position = CANDY_LOCKER_CONFIG_START + 4 + (index as usize) * PASS_CODE_LINE_SIZE;

        let length_till_pass_codes = CANDY_LOCKER_CONFIG_START;

        let array_slice: &mut [u8] =
            &mut data[position..position + fixed_pass_code_lines.len() * PASS_CODE_LINE_SIZE];

        array_slice.copy_from_slice(serialized);

        let mut new_count = current_pass_code_count;
        for i in 0..fixed_pass_code_lines.len() {
            msg!("Increasing count");
            new_count = new_count
                .checked_add(1)
                .ok_or(ErrorCode::NumericalOverflowError)?;
        ...
        // plug in new count.
        data[CANDY_LOCKER_CONFIG_START..CANDY_LOCKER_CONFIG_START + 4]
            .copy_from_slice(&(new_count as u32).to_le_bytes());
    ...
    pub fn initialize_candy_locker(
        ctx: Context<InitializeCandyLocker>
    ) -> ProgramResult {
        let candy_machine_account = &mut ctx.accounts.candy_machine;
        let candy_locker_account = &mut ctx.accounts.candy_locker;
        let authority = &mut ctx.accounts.authority;
        let mut candy_machine = CandyLocker {
            authority: ctx.accounts.authority.key(),
            passes_used: 0,
        };

        // let mut new_data = CandyLocker::discriminator().try_to_vec().unwrap();
        // new_data.append(&mut candy_machine.try_to_vec().unwrap());
        // let mut data = candy_machine_account.data.borrow_mut();
...
fn get_space_for_candy_locker(total_pass_code: u32) -> core::result::Result<usize, ProgramError> {
    let num = (total_pass_code as usize) * PASS_CODE_LINE_SIZE;
...
/// Create a new candy locker instance.
#[derive(Accounts)]
#[instruction(pass_total: u32)]
pub struct InitializeCandyLocker<'info> {
    #[account(has_one = authority)]
    candy_machine: Account<'info, CandyMachine>,
    /// CHECK: account constraints checked in account trait
    #[account(zero, rent_exempt = skip, constraint = candy_locker.to_account_info().owner == program_id && candy_locker.to_account_info().data_len() >= get_space_for_candy_locker(pass_total)?)]
    candy_locker: UncheckedAccount<'info>,
    /// CHECK: wallet can be any account and is not written to or read
    wallet: UncheckedAccount<'info>,
    /// CHECK: authority can be any account and is not written to or read
    authority: UncheckedAccount<'info>,
...
/// Add multiple pass code lines to the candy machine.
#[derive(Accounts)]
pub struct AddPassCodeLine<'info> {
    #[account(mut, has_one = authority)]
    candy_machine: Account<'info, CandyMachine>,
    #[account(mut, has_one = authority)]
    candy_locker: Account<'info, CandyLocker>,
    authority: Signer<'info>,
}
...
#[derive(Accounts)]
#[instruction(creator_bump: u8, pass_code: Option<String>)]
...
pub struct UpdateCandyMachine<'info> {
    ...
    /// CHECK: wallet can be any account and is not written to or read
    wallet: UncheckedAccount<'info>,
    // > Only needed if candy machine has candy locker
    // optional candy locker account passed here
}
...
pub const CANDY_LOCKER_CONFIG_START: usize = 8 + // key
    32 + // authority
    8  // passes_used
;
...
#[account]
#[derive(Default)]
pub struct CandyLocker {
    pub authority: Pubkey,
    pub passes_used: u64,
    // pub data: PassCodeLine,
    // there's a borsh vec u32 denoting how many actual lines of pass codes there are currently (eventually equals total pass codes available)
    // pub encrypted_pass: Option<PassCodeLine>,
}
...
pub fn get_pass_code_count(
    data: &RefMut<&mut [u8]>,
) -> core::result::Result<usize, ProgramError> {
    return Ok(u32::from_le_bytes(*array_ref![data, CANDY_LOCKER_CONFIG_START, 4]) as usize);
}
...
/// Individual pass_code line for storing code pre-mint.
pub const PASS_CODE_LINE_SIZE: usize = 33;
#[derive(AnchorSerialize, AnchorDeserialize, Debug)]
pub struct PassCodeLine {
    pub pass_code_encrypted: [u8; 32],
    // bool determining if the code has been used or not
    pub used: bool,
}
...
pub enum ErrorCode {
    ...
    #[msg("All pass codes has been consumed")]
    AllPassesConsumed,
    #[msg("Pass has code has been used")]
    PassCodeHasBeenUsed,
    #[msg("Incorrect pass code")]
    PassCodeNotFound,
    #[msg("No pass code supplied")]
    PassCodeNotRecieved,
    #[msg("Candy locker not supplied")]
